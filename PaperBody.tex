...
Table \ref{table:Recall} and figure \ref{fig:EclipseHeatmap} show the results of the EX3. In table \ref{table:Recall}, we presented the recall of each algorithms. The recall is the ratio between the number of $1$s in the decision vector $\textbf{x}$ and the number of resolved requirements in the real dataset. In ECLIPSE, FIREFOX-I, and FIREFOX-III, we can verify that each algorithms has a high recall values above 0.75 and the recall value increases as higher maximum profit was got in EX1. It can be easily confirmed on figure \ref{fig:EclipseHeatmap}. Therefore, we can concludet that the profit strategy of ECLIPSE, FIREFOX-I and FIREFOX-III are similar to the real world dataset.

However, in FIREFOX-II, all algorithms had lower recall values than other datasets. But we can see that AHP has much higher recall values than other algorithms. 


In the table, we can verify that it has a high recall value as higher maximum profit in EX1, except FIREFOX-II. 
...
FIREFOX-III & 0.68268 & 0.70486 & 0.74155 & 0.76637 & 0.78300\\\bottomrule
\end{tabular}
\label{table:Recall}
\end{table*}
